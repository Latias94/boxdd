...
#[test]
fn raw_ids_bind_only_to_their_live_world() {
    let mut world = World::new(WorldDef::default()).unwrap();
    ...
    assert_eq!(world.bind_body_id(body.unbind()), Ok(body));
    assert_eq!(world.bind_shape_id(shape.unbind()), Ok(shape));

    let other_world = World::new(WorldDef::default()).unwrap();
    assert_eq!(
        other_world.bind_body_id(body.unbind()),
        Err(ApiError::WrongWorld)
    );
    assert_eq!(
        other_world.bind_shape_id(shape.unbind()),
        Err(ApiError::WrongWorld)
    );
}
