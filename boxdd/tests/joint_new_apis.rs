...
#[test]
fn joint_base_new_matches_upstream_defaults() {
    let mut world = World::new(WorldDef::default()).unwrap();
    let body_a = create_dynamic_body(&mut world, [-1.0, 0.0]);
    let body_b = create_dynamic_body(&mut world, [1.0, 0.0]);
...
fn joint_defs_are_readable_value_types() {
    ...
    let rebuilt_base = base.with_draw_scale(2.0);
    ...
    let distance = DistanceJointDef::new(base)
        .length(3.5)
        .enable_spring(true)
    ...
    let prismatic = PrismaticJointDef::new(base)
        .enable_spring(true)
        .hertz(7.0)
        .damping_ratio(0.4)
        .translation(0.125)
    ...
    assert!(approx_eq(prismatic.target_translation(), 0.125, 1.0e-6));
    ...
    let revolute = RevoluteJointDef::new(base)
        .target_angle(0.2)
        .enable_spring(true)
        .hertz(8.0)
        .damping_ratio(0.3)
    ...
    let weld = WeldJointDef::new(base)
        .linear_hertz(3.0)
        .angular_hertz(4.0)
    ...
    let wheel = WheelJointDef::new(base)
        .enable_spring(true)
        .hertz(6.0)
        .damping_ratio(0.5)
        .enable_limit(true)
    ...
    let motor = MotorJointDef::new(base)
    ...
    let filter = FilterJointDef::new(base);
    assert!(same_body_id(filter.base().body_b_id(), body_b));
    assert!(filter.base().collide_connected());
    filter.validate().unwrap();

    let distance_id = world.create_distance_joint_id(&distance);
    assert_eq!(world.joint_type(distance_id), JointType::Distance);
    assert!(approx_eq(world.distance_length(distance_id), 3.5, 1.0e-6));
    assert!(approx_eq(
        world.distance_spring_hertz(distance_id),
        5.0,
        1.0e-6
    ));

    let prismatic_id = world.create_prismatic_joint_id(&prismatic);
    assert_eq!(world.joint_type(prismatic_id), JointType::Prismatic);
    assert!(approx_eq(
        world.prismatic_target_translation(prismatic_id),
        0.125,
        1.0e-6
    ));
    assert!(approx_eq(
        world.prismatic_max_motor_force(prismatic_id),
        11.0,
        1.0e-6
    ));

    let revolute_id = world.create_revolute_joint_id(&revolute);
    assert_eq!(world.joint_type(revolute_id), JointType::Revolute);
    assert!(approx_eq(
        world.revolute_target_angle(revolute_id),
        0.2,
        1.0e-6
    ));

    let weld_id = world.create_weld_joint_id(&weld);
    assert_eq!(world.joint_type(weld_id), JointType::Weld);
    assert!(approx_eq(world.weld_linear_hertz(weld_id), 3.0, 1.0e-6));
    assert!(approx_eq(
        world.weld_angular_damping_ratio(weld_id),
        0.7,
        1.0e-6
    ));

    let wheel_id = world.create_wheel_joint_id(&wheel);
    assert_eq!(world.joint_type(wheel_id), JointType::Wheel);
    assert!(approx_eq(world.wheel_spring_hertz(wheel_id), 6.0, 1.0e-6));
    assert!(approx_eq(
        world.wheel_max_motor_torque(wheel_id),
        7.0,
        1.0e-6
    ));

    let motor_id = world.create_motor_joint_id(&motor);
    assert_eq!(world.joint_type(motor_id), JointType::Motor);
    assert!(approx_vec2(
        world.motor_linear_velocity(motor_id),
        Vec2::new(2.0, -1.0),
        1.0e-6
    ));
    assert!(approx_eq(
        world.motor_max_spring_torque(motor_id),
        9.0,
        1.0e-6
    ));

    let filter_id = world.create_filter_joint_id(&filter);
    assert_eq!(world.joint_type(filter_id), JointType::Filter);
    assert!(world.joint_collide_connected(filter_id));
}
...
fn joint_runtime_metadata_and_tuning_are_available_across_owned_scoped_and_world_apis() {
    ...
    let base = JointBase::new(body_a, body_b)
        .with_local_frames(expected_frame_a, expected_frame_b)
        .with_collide_connected(true)
        .with_force_threshold(initial_force_threshold)
        .with_torque_threshold(initial_torque_threshold)
    ...
    let joint_id = world.create_distance_joint_id(
        &DistanceJointDef::new(
            JointBase::new(body_a, body_b)
                ...
                .with_torque_threshold(torque_threshold)
                .with_constraint_tuning(tuning),
        )
        .length(3.5),
    );
...
fn distance_joint_runtime_specific_apis_are_available_across_handle_types() {
    ...
    let handle = world.handle();
    let joint_ids = handle.body_joints(body_a);
    assert!(joint_ids.contains(&joint_id));
    assert!(approx_eq(
        handle.distance_length(joint_id),
        world.distance_length(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_length(joint_id).unwrap(),
        world.distance_length(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.distance_spring_enabled(joint_id),
        world.distance_spring_enabled(joint_id)
    );
    assert_eq!(
        handle.try_distance_spring_enabled(joint_id).unwrap(),
        world.distance_spring_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.distance_lower_spring_force(joint_id),
        world.distance_lower_spring_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_lower_spring_force(joint_id).unwrap(),
        world.distance_lower_spring_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_upper_spring_force(joint_id),
        world.distance_upper_spring_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_upper_spring_force(joint_id).unwrap(),
        world.distance_upper_spring_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_spring_hertz(joint_id),
        world.distance_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_spring_hertz(joint_id).unwrap(),
        world.distance_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_spring_damping_ratio(joint_id),
        world.distance_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_spring_damping_ratio(joint_id).unwrap(),
        world.distance_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.distance_limit_enabled(joint_id),
        world.distance_limit_enabled(joint_id)
    );
    assert_eq!(
        handle.try_distance_limit_enabled(joint_id).unwrap(),
        world.distance_limit_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.distance_min_length(joint_id),
        world.distance_min_length(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_min_length(joint_id).unwrap(),
        world.distance_min_length(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_max_length(joint_id),
        world.distance_max_length(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_max_length(joint_id).unwrap(),
        world.distance_max_length(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_current_length(joint_id),
        world.distance_current_length(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_current_length(joint_id).unwrap(),
        world.distance_current_length(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.distance_motor_enabled(joint_id),
        world.distance_motor_enabled(joint_id)
    );
    assert_eq!(
        handle.try_distance_motor_enabled(joint_id).unwrap(),
        world.distance_motor_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.distance_motor_speed(joint_id),
        world.distance_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_motor_speed(joint_id).unwrap(),
        world.distance_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_max_motor_force(joint_id),
        world.distance_max_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_max_motor_force(joint_id).unwrap(),
        world.distance_max_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.distance_motor_force(joint_id),
        world.distance_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_distance_motor_force(joint_id).unwrap(),
        world.distance_motor_force(joint_id),
        1.0e-6
    ));
}
...
fn prismatic_joint_runtime_specific_apis_are_available_across_handle_types() {
    ...
    assert!(joint_ids.contains(&joint_id));
    assert_eq!(
        handle.prismatic_spring_enabled(joint_id),
        world.prismatic_spring_enabled(joint_id)
    );
    assert_eq!(
        handle.try_prismatic_spring_enabled(joint_id).unwrap(),
        world.prismatic_spring_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.prismatic_spring_hertz(joint_id),
        world.prismatic_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_spring_hertz(joint_id).unwrap(),
        world.prismatic_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_spring_damping_ratio(joint_id),
        world.prismatic_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_spring_damping_ratio(joint_id).unwrap(),
        world.prismatic_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_target_translation(joint_id),
        world.prismatic_target_translation(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_target_translation(joint_id).unwrap(),
        world.prismatic_target_translation(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.prismatic_limit_enabled(joint_id),
        world.prismatic_limit_enabled(joint_id)
    );
    assert_eq!(
        handle.try_prismatic_limit_enabled(joint_id).unwrap(),
        world.prismatic_limit_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.prismatic_lower_limit(joint_id),
        world.prismatic_lower_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_lower_limit(joint_id).unwrap(),
        world.prismatic_lower_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_upper_limit(joint_id),
        world.prismatic_upper_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_upper_limit(joint_id).unwrap(),
        world.prismatic_upper_limit(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.prismatic_motor_enabled(joint_id),
        world.prismatic_motor_enabled(joint_id)
    );
    assert_eq!(
        handle.try_prismatic_motor_enabled(joint_id).unwrap(),
        world.prismatic_motor_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.prismatic_motor_speed(joint_id),
        world.prismatic_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_motor_speed(joint_id).unwrap(),
        world.prismatic_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_max_motor_force(joint_id),
        world.prismatic_max_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_max_motor_force(joint_id).unwrap(),
        world.prismatic_max_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_motor_force(joint_id),
        world.prismatic_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_motor_force(joint_id).unwrap(),
        world.prismatic_motor_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_translation(joint_id),
        world.prismatic_translation(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_translation(joint_id).unwrap(),
        world.prismatic_translation(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.prismatic_speed(joint_id),
        world.prismatic_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_prismatic_speed(joint_id).unwrap(),
        world.prismatic_speed(joint_id),
        1.0e-6
    ));
}
...
fn revolute_joint_runtime_specific_apis_are_available_across_handle_types() {
    ...
    assert!(joint_ids.contains(&joint_id));
    assert_eq!(
        handle.revolute_spring_enabled(joint_id),
        world.revolute_spring_enabled(joint_id)
    );
    assert_eq!(
        handle.try_revolute_spring_enabled(joint_id).unwrap(),
        world.revolute_spring_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.revolute_spring_hertz(joint_id),
        world.revolute_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_spring_hertz(joint_id).unwrap(),
        world.revolute_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.revolute_spring_damping_ratio(joint_id),
        world.revolute_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_spring_damping_ratio(joint_id).unwrap(),
        world.revolute_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.revolute_target_angle(joint_id),
        world.revolute_target_angle(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_target_angle(joint_id).unwrap(),
        world.revolute_target_angle(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.revolute_angle(joint_id),
        world.revolute_angle(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_angle(joint_id).unwrap(),
        world.revolute_angle(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.revolute_limit_enabled(joint_id),
        world.revolute_limit_enabled(joint_id)
    );
    assert_eq!(
        handle.try_revolute_limit_enabled(joint_id).unwrap(),
        world.revolute_limit_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.revolute_lower_limit(joint_id),
        world.revolute_lower_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_lower_limit(joint_id).unwrap(),
        world.revolute_lower_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.revolute_upper_limit(joint_id),
        world.revolute_upper_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_upper_limit(joint_id).unwrap(),
        world.revolute_upper_limit(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.revolute_motor_enabled(joint_id),
        world.revolute_motor_enabled(joint_id)
    );
    assert_eq!(
        handle.try_revolute_motor_enabled(joint_id).unwrap(),
        world.revolute_motor_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.revolute_motor_speed(joint_id),
        world.revolute_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_motor_speed(joint_id).unwrap(),
        world.revolute_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.revolute_motor_torque(joint_id),
        world.revolute_motor_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_motor_torque(joint_id).unwrap(),
        world.revolute_motor_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.revolute_max_motor_torque(joint_id),
        world.revolute_max_motor_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_revolute_max_motor_torque(joint_id).unwrap(),
        world.revolute_max_motor_torque(joint_id),
        1.0e-6
    ));
}
...
fn weld_joint_runtime_specific_apis_are_available_across_handle_types() {
    ...
    assert!(joint_ids.contains(&joint_id));
    assert!(approx_eq(
        handle.weld_linear_hertz(joint_id),
        world.weld_linear_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_weld_linear_hertz(joint_id).unwrap(),
        world.weld_linear_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.weld_linear_damping_ratio(joint_id),
        world.weld_linear_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_weld_linear_damping_ratio(joint_id).unwrap(),
        world.weld_linear_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.weld_angular_hertz(joint_id),
        world.weld_angular_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_weld_angular_hertz(joint_id).unwrap(),
        world.weld_angular_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.weld_angular_damping_ratio(joint_id),
        world.weld_angular_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_weld_angular_damping_ratio(joint_id).unwrap(),
        world.weld_angular_damping_ratio(joint_id),
        1.0e-6
    ));
}
...
fn wheel_joint_runtime_specific_apis_are_available_across_handle_types() {
    ...
    assert!(joint_ids.contains(&joint_id));
    assert_eq!(
        handle.wheel_spring_enabled(joint_id),
        world.wheel_spring_enabled(joint_id)
    );
    assert_eq!(
        handle.try_wheel_spring_enabled(joint_id).unwrap(),
        world.wheel_spring_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.wheel_spring_hertz(joint_id),
        world.wheel_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_spring_hertz(joint_id).unwrap(),
        world.wheel_spring_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.wheel_spring_damping_ratio(joint_id),
        world.wheel_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_spring_damping_ratio(joint_id).unwrap(),
        world.wheel_spring_damping_ratio(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.wheel_limit_enabled(joint_id),
        world.wheel_limit_enabled(joint_id)
    );
    assert_eq!(
        handle.try_wheel_limit_enabled(joint_id).unwrap(),
        world.wheel_limit_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.wheel_lower_limit(joint_id),
        world.wheel_lower_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_lower_limit(joint_id).unwrap(),
        world.wheel_lower_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.wheel_upper_limit(joint_id),
        world.wheel_upper_limit(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_upper_limit(joint_id).unwrap(),
        world.wheel_upper_limit(joint_id),
        1.0e-6
    ));
    assert_eq!(
        handle.wheel_motor_enabled(joint_id),
        world.wheel_motor_enabled(joint_id)
    );
    assert_eq!(
        handle.try_wheel_motor_enabled(joint_id).unwrap(),
        world.wheel_motor_enabled(joint_id)
    );
    assert!(approx_eq(
        handle.wheel_motor_speed(joint_id),
        world.wheel_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_motor_speed(joint_id).unwrap(),
        world.wheel_motor_speed(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.wheel_motor_torque(joint_id),
        world.wheel_motor_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_motor_torque(joint_id).unwrap(),
        world.wheel_motor_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.wheel_max_motor_torque(joint_id),
        world.wheel_max_motor_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_wheel_max_motor_torque(joint_id).unwrap(),
        world.wheel_max_motor_torque(joint_id),
        1.0e-6
    ));
}
...
fn motor_joint_runtime_specific_apis_are_available_across_handle_types() {
    ...
    assert!(joint_ids.contains(&joint_id));
    assert!(approx_vec2(
        handle.motor_linear_velocity(joint_id),
        world.motor_linear_velocity(joint_id),
        1.0e-6
    ));
    assert!(approx_vec2(
        handle.try_motor_linear_velocity(joint_id).unwrap(),
        world.motor_linear_velocity(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_angular_velocity(joint_id),
        world.motor_angular_velocity(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_angular_velocity(joint_id).unwrap(),
        world.motor_angular_velocity(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_max_velocity_force(joint_id),
        world.motor_max_velocity_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_max_velocity_force(joint_id).unwrap(),
        world.motor_max_velocity_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_max_velocity_torque(joint_id),
        world.motor_max_velocity_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_max_velocity_torque(joint_id).unwrap(),
        world.motor_max_velocity_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_linear_hertz(joint_id),
        world.motor_linear_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_linear_hertz(joint_id).unwrap(),
        world.motor_linear_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_linear_damping_ratio(joint_id),
        world.motor_linear_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_linear_damping_ratio(joint_id).unwrap(),
        world.motor_linear_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_angular_hertz(joint_id),
        world.motor_angular_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_angular_hertz(joint_id).unwrap(),
        world.motor_angular_hertz(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_angular_damping_ratio(joint_id),
        world.motor_angular_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_angular_damping_ratio(joint_id).unwrap(),
        world.motor_angular_damping_ratio(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_max_spring_force(joint_id),
        world.motor_max_spring_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_max_spring_force(joint_id).unwrap(),
        world.motor_max_spring_force(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.motor_max_spring_torque(joint_id),
        world.motor_max_spring_torque(joint_id),
        1.0e-6
    ));
    assert!(approx_eq(
        handle.try_motor_max_spring_torque(joint_id).unwrap(),
        world.motor_max_spring_torque(joint_id),
        1.0e-6
    ));
}
