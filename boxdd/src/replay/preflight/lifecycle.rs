...
use super::{
    Argument, ArgumentTag, PreflightError, ReturnKind, TailKind, generated::OperationRule,
};
...
impl Pool {
    fn from_snapshot(snapshot: &SnapshotValidation, kind: u32) -> Result<Self, PreflightError> {
        let pool_index = usize::try_from(kind - 1).map_err(|_| PreflightError::ContractMismatch)?;
        let next = usize::try_from(snapshot.facts.pool_next[pool_index])
            .map_err(|_| PreflightError::ContractMismatch)?;
        ...
        let base = snapshot.facts.pool_next[..pool_index]
            ...
            .ok_or(PreflightError::ContractMismatch)?;
        ...
        for (index, entry) in entries.iter().enumerate() {
            if entry.kind != kind
                || entry.index
                    != i32::try_from(index).map_err(|_| PreflightError::ContractMismatch)?
    ...
    fn allocate(&mut self, opcode: u8) -> Result<(usize, u16), PreflightError> {
        ...
        let slot = self
            .slots
            .get_mut(index)
            .ok_or(PreflightError::InvalidLifecycle(opcode))?;
        if slot.live {
            return Err(PreflightError::InvalidLifecycle(opcode));
        }
    ...
    fn require(&self, id: ObjectId, opcode: u8) -> Result<usize, PreflightError> {
        let index = id
            .index1
            .checked_sub(1)
            .and_then(|value| usize::try_from(value).ok())
            .ok_or(PreflightError::InvalidReference(opcode))?;
        let slot = self
            .slots
            .get(index)
            .ok_or(PreflightError::InvalidReference(opcode))?;
        if !slot.live || slot.generation != id.generation {
            return Err(PreflightError::InvalidReference(opcode));
        }
        Ok(index)
    }

    fn release(&mut self, index: usize, opcode: u8) -> Result<(), PreflightError> {
        let slot = self
            .slots
            .get_mut(index)
            .ok_or(PreflightError::InvalidLifecycle(opcode))?;
...
impl Lifecycle {
    pub(super) fn from_snapshot(snapshot: &SnapshotValidation) -> Result<Self, PreflightError> {
    ...
    pub(super) fn observe(&mut self, observation: Observation<'_>) -> Result<(), PreflightError> {
        let Observation {
            rule,
            return_kind,
            tail_kind,
    ...
    pub(super) fn finish(&self) -> Result<(), PreflightError> {
    ...
    fn load_snapshot_metadata(
        ...
        for entry in &snapshot.entries {
            ...
            let index =
                usize::try_from(entry.index).map_err(|_| PreflightError::ContractMismatch)?;
            match entry.kind {
                SNAPSHOT_ENTRY_BODY => {
                    let body = self
                        .bodies
        ...
        for (shape_index, shape) in self.shapes.iter().enumerate() {
            ...
            let chain_index = usize::try_from(shape.parent_chain)
                .map_err(|_| PreflightError::ContractMismatch)?;
            let order =
                usize::try_from(shape.chain_order).map_err(|_| PreflightError::ContractMismatch)?;
            let target = self
                .chains
                .get_mut(chain_index)
            ...
            *target = i32::try_from(shape_index).map_err(|_| PreflightError::ContractMismatch)?;
        }
        Ok(())
    }

    fn validate_snapshot_graph(&mut self) -> Result<(), PreflightError> {
        ...
        for body in 0..self.bodies.len() {
            ...
            let body_i32 = i32::try_from(body).map_err(|_| PreflightError::ContractMismatch)?;

            let mut shape = shape_heads[body];
            let mut previous = NULL_INDEX;
            let mut observed_shapes = 0usize;
            while shape != NULL_INDEX {
                let index = usize::try_from(shape).map_err(|_| PreflightError::ContractMismatch)?;
                if shape_seen[index]
            ...
            while chain != NULL_INDEX {
                let index = usize::try_from(chain).map_err(|_| PreflightError::ContractMismatch)?;
            ...
            while key != NULL_INDEX {
                ...
                let seen_index =
                    usize::try_from(key).map_err(|_| PreflightError::ContractMismatch)?;
    ...
    fn require_world(
        ...
        match self.world {
            ...
            Some(_) => Err(PreflightError::InvalidReference(opcode)),
            None if may_initialize => {
                self.world = Some(id);
                Ok(())
            }
            None => Err(PreflightError::InvalidLifecycle(opcode)),
        }
    }

    fn require_object_world(&self, id: ObjectId, opcode: u8) -> Result<(), PreflightError> {
        let world = self.world.ok_or(PreflightError::InvalidLifecycle(opcode))?;
    ...
    fn require_body(&self, id: ObjectId, opcode: u8) -> Result<usize, PreflightError> {
        self.require_object_world(id, opcode)?;
        self.bodies_pool.require(id, opcode)
    }

    fn require_shape(&self, id: ObjectId, opcode: u8) -> Result<usize, PreflightError> {
        self.require_object_world(id, opcode)?;
        self.shapes_pool.require(id, opcode)
    }

    fn require_chain(&self, id: ObjectId, opcode: u8) -> Result<usize, PreflightError> {
        self.require_object_world(id, opcode)?;
        self.chains_pool.require(id, opcode)
    }

    fn require_joint(&self, id: ObjectId, opcode: u8) -> Result<usize, PreflightError> {
        self.require_object_world(id, opcode)?;
    ...
    fn create_body(
        ...
        self.bodies[index] = BodyState {
            body_type: u32::try_from(body_type).map_err(|_| PreflightError::InvalidEnum(opcode))?,
            ..BodyState::default()
        };
    ...
    fn allocate_shape(
        ...
        shape_type: u32,
        opcode: u8,
    ) -> Result<(usize, u16), PreflightError> {
        ...
        let index_i32 =
            i32::try_from(index).map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
        if old_head != NULL_INDEX {
            let old =
                usize::try_from(old_head).map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
            self.shapes[old].prev = index_i32;
        }
        self.shapes[index] = ShapeState {
            body: i32::try_from(body).map_err(|_| PreflightError::InvalidLifecycle(opcode))?,
            parent_chain,
            chain_order,
            prev: NULL_INDEX,
            next: old_head,
    ...
    fn create_chain(
        ...
        let shape_count = if is_loop {
            ...
            point_count
                .checked_sub(3)
                .ok_or(PreflightError::InvalidCount(opcode))?
        };
        let (chain_index, generation) = self.chains_pool.allocate(opcode)?;
        ensure_len(&mut self.chains, chain_index)?;
        let chain_i32 =
            i32::try_from(chain_index).map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
        ...
        for order in 0..shape_count {
            let (shape, _) = self.allocate_shape(
                body,
                chain_i32,
                i32::try_from(order).map_err(|_| PreflightError::InvalidLifecycle(opcode))?,
                CHAIN_SEGMENT_SHAPE,
                opcode,
            )?;
            shapes
                .push(i32::try_from(shape).map_err(|_| PreflightError::InvalidLifecycle(opcode))?);
        }
    ...
    fn create_joint(
        ...
        self.joints[index] = JointState {
            bodies: [
                i32::try_from(body_a).map_err(|_| PreflightError::InvalidLifecycle(opcode))?,
                i32::try_from(body_b).map_err(|_| PreflightError::InvalidLifecycle(opcode))?,
            ],
            prev: [NULL_INDEX; 2],
            next: [next_a, next_b],
            joint_type,
    ...
    fn destroy_body(&mut self, index: usize, opcode: u8) -> Result<(), PreflightError> {
        ...
        while self.bodies[index].head_shape != NULL_INDEX {
            let shape = usize::try_from(self.bodies[index].head_shape)
                .map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
    ...
    fn destroy_shape_internal(&mut self, index: usize, opcode: u8) -> Result<(), PreflightError> {
        let shape = self.shapes[index];
        let body =
            usize::try_from(shape.body).map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
        if shape.prev != NULL_INDEX {
    ...
    fn destroy_chain(&mut self, index: usize, opcode: u8) -> Result<(), PreflightError> {
        let body = usize::try_from(self.chains[index].body)
            .map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
        let index_i32 =
            i32::try_from(index).map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
        ...
        for shape in shapes {
            let shape =
                usize::try_from(shape).map_err(|_| PreflightError::InvalidLifecycle(opcode))?;
            if !self
                .shapes_pool
                .slots
                .get(shape)
    ...
    fn destroy_joint(&mut self, index: usize, opcode: u8) -> Result<(), PreflightError> {
        let joint = self.joints[index];
        for edge in 0..2 {
    ...
    fn require_live_index(&self, pool: &Pool, index: i32) -> Result<usize, PreflightError> {
        let index = usize::try_from(index).map_err(|_| PreflightError::ContractMismatch)?;
    ...
    fn decode_edge_key(&self, key: i32) -> Result<(usize, usize), PreflightError> {
        if key < 0 {
            return Err(PreflightError::ContractMismatch);
        }
        let joint = usize::try_from(key >> 1).map_err(|_| PreflightError::ContractMismatch)?;
        let edge = usize::try_from(key & 1).map_err(|_| PreflightError::ContractMismatch)?;
        if !self
            .joints_pool
            .slots
            .get(joint)
...
fn edge_key(joint: usize, edge: usize) -> Result<i32, PreflightError> {
    if edge > 1 {
        return Err(PreflightError::ContractMismatch);
    }
    joint
...
fn filled_vec<T: Clone>(length: usize, value: T) -> Result<Vec<T>, PreflightError> {
...
fn ensure_len<T: Default>(values: &mut Vec<T>, index: usize) -> Result<(), PreflightError> {
    if index >= values.len() {
        values
...
fn read_chain_def(bytes: &[u8], opcode: u8) -> Result<(usize, usize, bool), PreflightError> {
    let point_count = usize::try_from(read_i32(bytes, 8, opcode)?)
        .map_err(|_| PreflightError::InvalidCount(opcode))?;
...
fn read_world_id(bytes: &[u8], opcode: u8) -> Result<WorldId, PreflightError> {
    let raw: [u8; 4] = bytes
        .try_into()
        .map_err(|_| PreflightError::InvalidReference(opcode))?;
...
fn read_object_id(bytes: &[u8], opcode: u8) -> Result<ObjectId, PreflightError> {
    read_object_id_at(bytes, 0, opcode)
}

fn read_object_id_at(bytes: &[u8], offset: usize, opcode: u8) -> Result<ObjectId, PreflightError> {
    let raw: [u8; 8] = bytes
        .get(
            offset
                ..offset
...
fn read_i32(bytes: &[u8], offset: usize, opcode: u8) -> Result<i32, PreflightError> {
    let raw = bytes
        .get(
            offset
                ..offset
...
impl<'a> TailCursor<'a> {
    ...
    fn u8(&mut self) -> Result<u8, PreflightError> {
        Ok(self.take(1)?[0])
    }

    fn u32(&mut self) -> Result<u32, PreflightError> {
        Ok(u32::from_le_bytes(self.take(4)?.try_into().map_err(
            |_| PreflightError::PayloadTruncated(self.opcode),
        )?))
    }

    fn object_id(&mut self) -> Result<ObjectId, PreflightError> {
    ...
    fn finish(self) -> Result<(), PreflightError> {
        if self.cursor == self.bytes.len() {
            ...
            Err(PreflightError::PayloadMismatch {
                opcode: self.opcode,
...
mod tests {
    ...
    fn pool_reuse_invalidates_the_old_generation_and_preserves_lifo_order() {
        ...
        assert_eq!(
            ...
            Err(PreflightError::InvalidReference(OPCODE))
        );
