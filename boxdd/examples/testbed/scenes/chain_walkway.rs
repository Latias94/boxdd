...
pub fn build(app: &mut super::PhysicsApp, ground: bd::types::BodyId) {
    if app.chain_mode == 0 {
        ...
        for i in 0..n {
            ...
            let _ = app
                .world
                .create_polygon_shape_for(b, &sdef, &bd::shapes::box_polygon(0.2, 0.2));
            app.created_shapes += 1;
            ids.push(b);
        }
        for (i, &b) in ids.iter().enumerate() {
            let phase = (i as f32) as f32 * 0.2;
        ...
        for i in 0..n {
            ...
            let _ = app
                .world
                .create_polygon_shape_for(b, &sdef, &bd::shapes::box_polygon(link_half, 0.12));
            app.created_shapes += 1;
