...
// Basic standalone manifold viewer with adjustable shape poses and sizes.

fn relative_transform(frame_a: bd::Transform, frame_b: bd::Transform) -> bd::Transform {
    bd::Transform::from_pos_angle(
...
pub fn tick(app: &mut super::PhysicsApp) {
    ...
    let points = m.points();
    let normal = frame_a.rotation().rotate_vec(m.normal);
    state.contact_count = points.len();
    state.normal_x = normal.x;
    state.normal_y = normal.y;
    if let Some(point) = points.first() {
        let world_point = frame_a.transform_point(point.point);
        state.point1_x = world_point.x;
        state.point1_y = world_point.y;
    ...
    if let Some(point) = points.get(1) {
        let world_point = frame_a.transform_point(point.point);
        state.point2_x = world_point.x;
        state.point2_y = world_point.y;
...
pub fn ui_params(app: &mut super::PhysicsApp, ui: &imgui::Ui) {
    ...
    changed |= ui.slider(
        "Mode (0=Poly,1=Circle,2=Capsule,3=Seg,4=SegCapsule)",
        0,
        4,
        &mut mode,
    );
    ...
    if app.manifold.mode == 0 {
        ...
        let mut rr = app.manifold.b_radius;
        changed |= ui.slider(
            "B (capsule): half length (uses B Half X)",
            0.1,
            2.0,
            &mut bhx,
        );
    ...
    if app.manifold.show_metrics {
        ui.text(format!("P0: separation={:.4}", app.manifold.separation1));
        if app.manifold.contact_count > 1 {
            ui.text(format!("P1: separation={:.4}", app.manifold.separation2));
        }
    }
