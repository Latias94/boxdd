use boxdd as bd;
use dear_imgui_rs as imgui;

pub fn build(app: &mut super::PhysicsApp, _ground: bd::types::BodyId) {
    // Circle, box, capsule on ground with mixed materials
    let s_circle = bd::ShapeDef::builder()
        .density(1.0)
        .material(
            bd::SurfaceMaterial::default()
                .with_friction(0.8)
                .expect("shape sample friction must be valid")
                .with_restitution(0.7)
                .expect("shape sample restitution must be valid"),
        )
        .build()
        .expect("valid testbed definition");
    let s_poly = bd::ShapeDef::builder()
        .density(1.0)
        .material(
            bd::SurfaceMaterial::default()
                .with_friction(0.4)
                .expect("shape sample friction must be valid")
                .with_restitution(0.1)
                .expect("shape sample restitution must be valid"),
        )
        .build()
        .expect("valid testbed definition");
    let s_caps = bd::ShapeDef::builder()
        .density(1.0)
        .material(
            bd::SurfaceMaterial::default()
                .with_friction(0.05)
                .expect("shape sample friction must be valid")
                .with_restitution(0.0)
                .expect("shape sample restitution must be valid"),
        )
        .build()
        .expect("valid testbed definition");
    let b_circle = app
        .world
        .create_body(
            bd::BodyBuilder::from(app.foundation.body_def())
                .body_type(bd::BodyType::Dynamic)
                .position([-4.0_f32, 6.0])
                .build()
                .expect("valid testbed definition"),
        )
        .expect("valid testbed operation");
    app.created_bodies += 1;
    let b_poly = app
        .world
        .create_body(
            bd::BodyBuilder::from(app.foundation.body_def())
                .body_type(bd::BodyType::Dynamic)
                .position([0.0_f32, 6.0])
                .build()
                .expect("valid testbed definition"),
        )
        .expect("valid testbed operation");
    app.created_bodies += 1;
    let b_caps = app
        .world
        .create_body(
            bd::BodyBuilder::from(app.foundation.body_def())
                .body_type(bd::BodyType::Dynamic)
                .position([4.0_f32, 6.0])
                .build()
                .expect("valid testbed definition"),
        )
        .expect("valid testbed operation");
    app.created_bodies += 1;
    let _ = app
        .world
        .body(b_circle)
        .expect("valid testbed operation")
        .create_circle(
            &s_circle,
            &bd::shapes::circle([0.0_f32, 0.0], 0.5).expect("testbed circle must be valid"),
        )
        .expect("valid testbed operation");
    app.created_shapes += 1;
    let _ = app
        .world
        .body(b_poly)
        .expect("valid testbed operation")
        .create_polygon(
            &s_poly,
            &bd::shapes::box_polygon(0.6, 0.4).expect("valid polygon geometry"),
        )
        .expect("valid testbed operation");
    app.created_shapes += 1;
    let _ = app
        .world
        .body(b_caps)
        .expect("valid testbed operation")
        .create_capsule(
            &s_caps,
            &bd::shapes::capsule([-0.6_f32, 0.0], [0.6, 0.0], 0.2)
                .expect("testbed capsule must be valid"),
        )
        .expect("valid testbed operation");
    app.created_shapes += 1;
}

pub fn tick(_app: &mut super::PhysicsApp, _events: Option<&bd::StepEventsSnapshot>) {}

pub fn ui_params(_app: &mut super::PhysicsApp, ui: &imgui::Ui) {
    ui.text("Spawns: circle, box, capsule (materials)");
}
