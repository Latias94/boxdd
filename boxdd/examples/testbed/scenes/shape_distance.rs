...
pub fn tick(app: &mut super::PhysicsApp) {
    ...
    app.sd_point_ax = point_a.x;
    app.sd_point_ay = point_a.y;
    app.sd_point_bx = point_b.x;
    app.sd_point_by = point_b.y;
}

pub fn ui_params(app: &mut super::PhysicsApp, ui: &imgui::Ui) {
    // Controls for shape A and shape B
    let mut ax = app.sd_a_x;
    let mut ay = app.sd_a_y;
    let mut aa = app.sd_a_angle;
    let mut ahx = app.sd_a_hx;
    let mut ahy = app.sd_a_hy;
    let mut ar = app.sd_a_radius;
    let mut bx = app.sd_b_x;
    let mut by = app.sd_b_y;
    let mut ba = app.sd_b_angle;
    let mut bhx = app.sd_b_hx;
    let mut bhy = app.sd_b_hy;
    let mut br = app.sd_b_radius;
    let changed = ui.slider("A.x", -10.0, 10.0, &mut ax)
    ...
    if changed {
        ...
        app.sd_a_hx = ahx;
        app.sd_a_hy = ahy;
        app.sd_a_radius = ar.max(0.0);
        app.sd_b_x = bx;
        app.sd_b_y = by;
        app.sd_b_angle = ba;
        app.sd_b_hx = bhx;
        app.sd_b_hy = bhy;
        app.sd_b_radius = br.max(0.0);
    }
    ui.text(format!(
