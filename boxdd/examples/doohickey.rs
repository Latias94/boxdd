...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    // Common defs
    let bdef_dyn = BodyBuilder::from(foundation.body_def()).body_type(BodyType::Dynamic);
    ...
    let w1 = world.create_body(bdef_dyn.clone().position(p_w1).build()?)?;
    let _ = world.body(w1)?.create_circle(&sdef_rr, &circle)?;
    let w2 = world.create_body(bdef_dyn.clone().position(p_w2).build()?)?;
    let _ = world.body(w2)?.create_circle(&sdef_rr, &circle)?;

    let b1 = world.create_body(bdef_dyn.clone().position(p_b1).build()?)?;
    let _ = world.body(b1)?.create_capsule(&sdef_rr, &bar)?;
    let b2 = world.create_body(bdef_dyn.clone().position(p_b2).build()?)?;
    let _ = world.body(b2)?.create_capsule(&sdef_rr, &bar)?;
    ...
    // Revolute joints at wheel centers
    {
        ...
        let _ = world.create_revolute_joint(&rdef)?;
    }
    {
    ...
    let pdef = PrismaticJointDef::new(base)
        ...
        .hertz(1.0)
        .damping_ratio(0.5);
    let _ = world.create_prismatic_joint(&pdef)?;

    // Step and report
    for _ in 0..240 {
        drop(world.step(1.0 / 60.0, 8)?);
    }
    let b1p = world.body(b1)?.position()?;
    let b2p = world.body(b2)?.position()?;
    println!(
