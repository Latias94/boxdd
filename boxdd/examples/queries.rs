...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let owned_hits = world.overlap_aabb(Position::ZERO, aabb, filter);

    let mut reused_hits = Vec::new();
    world.overlap_aabb_into(Position::ZERO, aabb, filter, &mut reused_hits);

    let mut visited_hits = 0usize;
    let visited_all = world.visit_overlap_aabb(Position::ZERO, aabb, filter, |_| {
        visited_hits += 1;
        true
    });

    let mut stopped_early = false;
    let completed = world.visit_overlap_aabb(Position::ZERO, aabb, filter, |_| {
        stopped_early = true;
        false
    });
