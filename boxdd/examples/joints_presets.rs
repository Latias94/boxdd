use boxdd::{BodyBuilder, DistanceJointDef, RevoluteJointDef, ShapeDef, Vec2, shapes};

fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let foundation = boxdd::Foundation::initialize_default()?;
    let def = boxdd::WorldBuilder::from(foundation.world_def())
        .gravity(Vec2::new(0.0, -9.8))
        .build()?;
    let mut world = foundation.create_world(def)?;

    // Two dynamic boxes
    let a = world.create_body(
        BodyBuilder::from(foundation.body_def())
            .position(Vec2::new(-1.0, 2.0))
            .build()?,
    )?;
    let b = world.create_body(
        BodyBuilder::from(foundation.body_def())
            .position(Vec2::new(1.0, 2.0))
            .build()?,
    )?;
    let sdef = ShapeDef::builder().density(1.0).build()?;
    let _sa = world.body(a)?.create_polygon(
        &sdef,
        &shapes::box_polygon(0.5, 0.5).expect("valid polygon geometry"),
    )?;
    let _sb = world.body(b)?.create_polygon(
        &sdef,
        &shapes::box_polygon(0.5, 0.5).expect("valid polygon geometry"),
    )?;

    // Distance joint (springy)
    let wa = Vec2::new(-1.0, 2.0);
    let wb = Vec2::new(1.0, 2.0);
    let base = world.joint_base_from_world_points(a, b, wa, wb)?;
    let ddef = DistanceJointDef::new(base)
        .enable_spring(true)
        .hertz(4.0)
        .damping_ratio(0.7)
        .length_from_world_points(wa, wb)?;
    let _dj = world.create_distance_joint(&ddef)?;

    // Revolute joint (limits + motor)
    let base2 = world.joint_base_from_world_points(a, b, wa, wa)?;
    let rdef = RevoluteJointDef::new(base2)
        .limit_deg(-30.0, 30.0)
        .enable_motor(true)
        .motor_speed_deg(90.0)
        .max_motor_torque(10.0);
    let _rj = world.create_revolute_joint(&rdef)?;

    for _ in 0..60 {
        drop(world.step(1.0 / 60.0, 4)?);
    }
    Ok(())
}
