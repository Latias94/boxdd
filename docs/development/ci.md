...
- `build-pages-wasm` rebuilds the example index, compiles the Bevy + egui testbed through `wasm-bindgen`, builds the Emscripten Box2D provider, and writes runtime assets in `docs/pages/wasm/generated` plus `docs/pages/bevy-testbed/generated`.
...
- Pages runtime: install `wasm32-unknown-unknown`, `wasm-bindgen-cli 0.2.126`, and Emscripten SDK, then run `cargo run -p xtask -- build-pages-wasm` and `cargo run -p xtask -- validate-pages`.
