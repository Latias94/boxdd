...
| Slice | Status | Notes |
| --- | --- | --- |
| creation, builders, value-object inspection, explicit raw conversion | safe-covered | `BodyDef` is readable, validates the Box2D value-side preconditions, owns its optional name, and keeps raw re-entry explicit via `unsafe from_raw(...)` / `into_raw_guard()`. |
...
| Seam | Status | Notes |
| --- | --- | --- |
| raw conversion on crate-owned value types | raw-only | Intentional low-level escape hatch. Pointer-free values retain direct conversions; pointer-bearing `BodyDef` requires `unsafe from_raw(...)` and returns an owned `into_raw_guard()` instead of a potentially dangling raw value. |
