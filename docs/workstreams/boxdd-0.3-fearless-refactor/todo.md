...
- [x] Extend polygon construction helpers with recoverable `try_*` entrypoints (`try_square_polygon`, `try_box_polygon`, `try_rounded_box_polygon`, `try_offset_*`, `try_polygon_from_points`) and add `Body::try_create_polygon_from_points` so hull/transform/extent validation can return `ApiError::InvalidArgument` instead of collapsing to `Option`-only convenience APIs.
- [x] Close the remaining world-level shape creation recoverability gap by adding `World::try_create_{circle,segment,capsule,polygon}_shape_for*` and aligning shape/chain creation with the crate's callback-lock contract (`InCallback` on `try_*`, panic on the default safe path).
- [x] Close the remaining scoped-body shape creation recoverability gap by adding `Body::try_create_{circle,segment,capsule,polygon}_shape` plus `try_create_box` / `try_create_*_simple`, so body-local creation convenience no longer falls back to panic-only control flow for invalid defs or geometry.
- [x] Close the remaining owned-body local shape creation gap by adding `OwnedBody::{create_*,try_create_*}_shape`, convenience `create_box` / `create_*_simple`, and `create_polygon_from_points`, then route `World` / `Body<'_>` / `OwnedBody` through one shared private creation helper layer so geometry-type additions cannot drift again.
- [x] Close the matching body-local chain creation gap by adding `Body::try_create_chain` plus `OwnedBody::{create_chain,try_create_chain}`, and route `World` / `Body<'_>` / `OwnedBody` chain creation through one shared private helper so callback/error behavior stays aligned.
- [x] Continue the owned/scoped duplication cleanup inside body-local creation by collapsing the remaining `Body` / `OwnedBody` shape and chain helper plumbing behind shared private wrapper helpers, so convenience creators (`create_box`, `create_*_simple`, `create_polygon_from_points`, `create_chain`) cannot drift after the new completeness pass.
- [x] Continue the world-level creation cleanup by deleting the obsolete `world_create_*_shape_for_impl` forwarding layer and collapsing `World::{create_*,try_create_*}_shape_for_owned` plus `create_chain_for_owned` behind one shared owned-handle wrapper helper, so world-level owned creation stays aligned with the now-shared body-attached helper path.
- [x] Close the remaining scoped-handle borrow recoverability gap by adding `World::try_body` / `try_shape` / `try_joint` / `try_chain`, so invalid ids and callback-locked borrow attempts no longer require `Option` probing plus side-channel callback assumptions.
...
- [x] Split the stabilized typed joint runtime layer out of `joints/mod.rs` into `joints/runtime_typed.rs` so family-specific runtime FFI shims, world/id helpers, and owned/scoped wrapper forwarding no longer live in the same monolithic file as joint definitions and creation.
- [x] Start splitting `joints/runtime_typed.rs` by family and extract the full `Distance` runtime slice into `joints/runtime_typed_distance.rs`, leaving the remaining families on the shared file until their interleaved sections are untangled.
- [x] Continue splitting `joints/runtime_typed.rs` by peeling the contiguous `Weld` / `Wheel` / `Motor` tail into `joints/runtime_typed_weld_wheel_motor.rs`, leaving `Prismatic` / `Revolute` as the only remaining interleaved typed-runtime families.
- [x] Finish the typed-runtime family split by deleting the last shared `runtime_typed.rs` staging file and moving the remaining `Prismatic` / `Revolute` runtime slices into dedicated `runtime_typed_prismatic.rs` and `runtime_typed_revolute.rs` modules.
- [x] Finish the typed-runtime per-family split by deleting the temporary `runtime_typed_weld_wheel_motor.rs` grouping file and moving `Weld`, `Wheel`, and `Motor` into dedicated `runtime_typed_weld.rs`, `runtime_typed_wheel.rs`, and `runtime_typed_motor.rs` modules.
- [x] Split `JointBase` / `JointBaseBuilder` value-object definitions out of `joints/base.rs` into a dedicated `joints/base_def.rs` module so common joint-handle runtime code no longer shares a file with definition-builder plumbing.
- [x] Split the remaining joint creation/validation layer out of `joints/mod.rs` into a dedicated creation module so the top-level joint entry file now mainly acts as the public module root plus shared lightweight helpers.
- [x] Split the oversized `query.rs` module into dedicated `query/types.rs`, `query/raw.rs`, `query/checked.rs`, and `query/world_api.rs` modules so value types, validation, raw FFI plumbing, and `World` / `WorldHandle` entrypoints stop cohabiting one monolithic file.
- [x] Split the oversized `body.rs` module into dedicated `body/definition.rs`, `body/runtime.rs`, `body/owned.rs`, and `body/scoped.rs` modules so body value objects, runtime helper plumbing, and owned/scoped handle wrappers stop cohabiting one monolithic file.
- [x] Continue decomposing `body/runtime.rs` by splitting contact/attachment extraction, typed user-data plumbing, and the shared owned/scoped runtime-handle trait into `body/runtime/{attachments,user_data,handle}.rs`, leaving the root focused on core raw/helper functions reused by `Body`, `OwnedBody`, and `WorldHandle`.
- [x] Split the `SurfaceMaterial` / `ShapeDef` / `ShapeDefBuilder` value-object block out of `shapes/mod.rs` into `shapes/definition.rs` so shape definitions stop living inside the already-oversized runtime-heavy shape root.
- [x] Split the public `Shape` / `OwnedShape` wrapper bodies and body-local shape creation entrypoints out of `shapes/mod.rs` into dedicated `shapes/scoped.rs`, `shapes/owned.rs`, and `shapes/creation.rs` modules so the shape root stops cohabiting value objects, wrapper methods, and creation convenience layers.
- [x] Split the remaining internal shape runtime / validation / creation helper layer out of `shapes/mod.rs` into `shapes/runtime.rs`, keeping the module root focused on public type definitions, re-exports, and shared imports for the shape subtree.
- [x] Continue decomposing `shapes/runtime.rs` by splitting validation, body-attached creation plumbing, and shape user-data/checked helper layers into `shapes/runtime/{validation,creation,user_data}.rs`, leaving the root focused on core runtime queries plus the shared handle trait.
- [x] Continue decomposing `joints/base.rs` by splitting the runtime-handle trait, joint user-data helper layer, and owned/scoped wrapper bodies into `joints/base/{runtime_handle,user_data,owned,scoped}.rs`, leaving the root focused on shared type definitions plus raw/common helper functions.
- [x] Continue decomposing `world/runtime.rs` by splitting read-only helper plumbing, step/tuning control, and callback registration/trampolines into `world/runtime/{reads,control,callbacks}.rs`, leaving the root as a thin coordination module reused by both `World` and `WorldHandle`.
- [x] Split the `WorldDef` / `WorldBuilder` / world-configuration validation layer out of `world.rs` into `world/definition.rs` so top-level world configuration stops cohabiting with callback plumbing and the main runtime surface.
- [x] Split lightweight world-owned value objects (`Counters`, `Profile`, `OwnedHandleCounts`, `OutstandingOwnedHandles`) out of `world.rs` into `world/metrics.rs` so the main world module no longer mixes runtime entrypoints with passive snapshot/stat structs.
- [x] Split `WorldHandle` / `CallbackWorld` definitions plus the full stored-read-only query layer out of `world.rs` into `world/handle.rs` so the main world module stops cohabiting mutable runtime entrypoints with the callback-safe/stored-query surface.
- [x] Split the callback-heavy `World` runtime control layer out of `world.rs` into `world/runtime.rs` so stepping, gravity/diagnostic getters, tuning helpers, and callback registration stop cohabiting with world creation and id/handle plumbing.
- [x] Split the world-owned creation/lifecycle layer out of `world.rs` into `world/creation.rs` so body/shape/chain creation, destroy helpers, and world-space joint-base builders stop living beside the remaining id-scoped runtime accessors.
- [x] Split the remaining body-by-id runtime getter/mutation surface out of `world.rs` into `world/body_api.rs` so the root no longer mixes body id helpers with unrelated world coordination code.
- [x] Split scoped id-borrow helpers (`body` / `shape` / `joint` / `chain` plus `try_*`) out of `world.rs` into `world/borrow.rs` so callback-sensitive borrow plumbing stays on one small internal path.
- [x] Split the remaining shape-by-id runtime control/query surface out of `world.rs` into `world/shape_api.rs` so shape id helpers stop bloating the root after the body/borrow splits.
- [x] Split inline `world` regression tests out of `world.rs` into `world/tests.rs` so the root can stay focused on the runtime coordination surface instead of mixing implementation and test code.
