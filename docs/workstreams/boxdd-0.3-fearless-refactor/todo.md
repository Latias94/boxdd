...
- [x] Extend standalone world-free geometry helpers with helper-specific validation contracts and recoverable `try_*` variants (`try_mass_data`, `try_aabb`, `try_contains_point`, `try_ray_cast`, `try_transformed`) so Rust-side checks match Box2D's actual helper preconditions without over-rejecting degenerate segment/capsule cases.
