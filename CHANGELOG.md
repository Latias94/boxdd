...
- Recoverable standalone collision entrypoints: `try_segment_distance`, `try_shape_distance`, `try_shape_cast`, `try_time_of_impact`, and `try_collide_*` manifold helpers, plus `validate()` helpers on `ShapeProxy`, `DistanceInput`, `ShapeCastPairInput`, `Sweep`, `ToiInput`, and crate-owned geometry values (`Circle`, `Segment`, `ChainSegment`, `Capsule`, `Polygon`).
...
- Explicit raw task-system configuration helpers on `WorldDef` / `WorldBuilder` so advanced multithreaded Box2D integration no longer requires direct field mutation on `ffi::b2WorldDef`.
